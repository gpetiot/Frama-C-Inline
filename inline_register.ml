...
(* Make substitutions in terms and predicates
 * 
 * Bindings:
 * ll: logic_label => logic_label
 * vt: logic_var => term
 * vp: logic_var => predicate
 * vv: logic_var => logic_var
 *)
...
class inline_inplace = object
  inherit Visitor.frama_c_inplace

  method! private vpredicate_named p =
    Cil.ChangeTo ((new subst ())#pnamed p [] [] [] [])
...
let run () = 
  if Inline_options.Enabled.get () then
    ...
    Inline_options.Self.feedback
      "inlined project named '%s'" inlined_project_name;
    let last = false in
